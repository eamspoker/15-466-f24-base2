...
Scene::Drawable::Pipeline transparent_color_texture_program_pipeline;

Load< TransparentColorTextureProgram > transparent_color_texture_program(LoadTagEarly, []() -> TransparentColorTextureProgram const * {
	TransparentColorTextureProgram *ret = new TransparentColorTextureProgram();

	//----- build the pipeline template -----
	transparent_color_texture_program_pipeline.program = ret->program;

	transparent_color_texture_program_pipeline.OBJECT_TO_CLIP_mat4 = ret->OBJECT_TO_CLIP_mat4;
	transparent_color_texture_program_pipeline.OBJECT_TO_LIGHT_mat4x3 = ret->OBJECT_TO_LIGHT_mat4x3;
	transparent_color_texture_program_pipeline.NORMAL_TO_LIGHT_mat3 = ret->NORMAL_TO_LIGHT_mat3;
	...
	*/
	...
	transparent_color_texture_program_pipeline.textures[0].texture = tex;
	transparent_color_texture_program_pipeline.textures[0].target = GL_TEXTURE_2D;

	return ret;
});

TransparentColorTextureProgram::TransparentColorTextureProgram() {
	...
	,
		...
		"		e =  (smoothstep(0.0,0.1,nl) * 0.6 + 0.4) * LIGHT_ENERGY;\n"
		...
		"      vec3 l = normalize(LIGHT_LOCATION - vec3(0.0)); \n"
		"      float nl = dot(-0.75*n,-0.75*l); \n"
		"	e =  (smoothstep(0.0,0.1,nl) * 0.6 + 0.4) * LIGHT_ENERGY;\n"
		"	}\n"
		"	vec4 albedo = texture(TEX, texCoord) * (color/3.1415926);\n"
		"     albedo.r += LIGHT_ENERGY.r*0.002;\n"
     	" 	albedo.g += LIGHT_ENERGY.g*0.3;\n"
     	"	albedo.b += LIGHT_ENERGY.b*0.75;\n"
		"	fragColor = vec4(e*albedo.rgb, 0.0);\n"
		"}\n"
	);
